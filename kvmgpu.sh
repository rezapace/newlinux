...
# Aktifkan IOMMU di kernel Linux
echo "Enabling IOMMU in grub..."
sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="quiet splash"/GRUB_CMDLINE_LINUX_DEFAULT="quiet splash intel_iommu=on"/g' /etc/default/grub
update-grub

# Isolasi GPU dari driver host
echo "Isolating GPU from host drivers..."
echo "options vfio-pci ids=10de:1b80" > /etc/modprobe.d/vfio.conf

# Tambahkan modul VFIO ke initramfs
